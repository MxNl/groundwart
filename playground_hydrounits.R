...
colour_title_light <- colour_panel
colour_subtitle_light <- "grey70"
...
data_gwl <- read_rds("groundwart_germany_hydrounits_data.rds")
key_well_meta <- read_rds("key_well_meta.rds")
key_well_meta <- key_well_meta |>
  filter(fk_well %in% unique(data_gwl$fk_well))

hydrounits <- sf::read_sf("J:/NUTZER/Noelscher.M/Studierende/Daten/hydrogeological_units/germany/time_invariant/shape/hyraum_v32_bgr/data/hyraum_tr__v32_poly.shp")

hydrounits |>
  ggplot() +
  geom_sf(colour = "white")

hydrounits_grid <- hydrounits |>
  geofacet::grid_auto()


hydrounits_grid |>
  grid_preview(label = "name_TR_NR_NAME")
grid_design(label = "name_R_NR_Name")



geofacet_grid_germany <- hydrounits_grid |>
  rename(name = name_TR_NR_NAME, code = code_TR_NR)
# filter(
...
data_gwl <- data_gwl |>
  left_join(key_well_meta |> select(fk_well, well_id)) |>
  mutate(gwl = if_else(week == lubridate::week(as.Date("1999-10-25")) & year == lubridate::year(as.Date("1999-10-25")) & well_id == "BW_119-813-3", NA_real_, gwl)) |>
  # mutate(gwl = if_else(between(date, as.Date("2022-12-05"), as.Date("2023-02-20")) & well_id == "HE_7297", NA_real_, gwl)) |>
  mutate(gwl = if_else(week == lubridate::week(as.Date("2015-07-20")) & year == lubridate::year(as.Date("2015-07-20")) & well_id == "SN_51410818", NA_real_, gwl))
...
data_plot <- data_gwl |>
  ...
  left_join(hydrounits |> sf::st_join(key_well_meta) |> sf::st_drop_geometry() |> select(code = TR_NR, fk_well), by = "fk_well") |>
  # rename(code = fk_well) |>
...
plot |>
  ggsave(filename = "groundwarter_singlewell_per_hydrounit_polarcoord_dark_a2.png", width = 59.4, height = 84.1, units = "cm")
...
plot <- data_plot |>
  ...
  geom_line(aes(alpha = year), linewidth = 0.6, show.legend = FALSE) +
  # with_outer_glow(geom_point(data = data_plot |> group_by(code) |> slice_max(order_by = year + week), colour = "white", shape = 8, show.legend = FALSE), colour = "white") +
  geom_point(data = data_plot |> group_by(code) |> slice_max(order_by = year + week), colour = colour_endpoint_light, shape = 8, show.legend = FALSE) +
  # geom_line(linewidth = .6, show.legend = FALSE) |> partition(vars(year)) |> blend("multiply") +
  scico::scale_colour_scico(
    limits = c(year_start, year_end),
    palette = "hawaii",
  ...
  #   panel.grid.minor.x = element_blank(),
  #   panel.grid.major.x = element_line(colour = colour_subtitle_light),
  #   axis.line = element_line(colour = colour_subtitle_light),
  #   axis.text = element_blank(),
  #   strip.background = element_blank(),
  #   strip.text = element_blank(),
  #   axis.ticks = element_line(colour = colour_panel_light),
...
plot |>
  ggsave(filename = "groundwarter_singlewell_per_hydrounit_polarcoord_light_a2.png", width = 59.4, height = 84.1, units = "cm")
...
plot |>
  ggsave(filename = "groundwarter_singlewell_per_hydrounits_referencecomparison_polar_dark_a2.png", width = 59.4, height = 84.1, units = "cm")
...
plot |>
  ggsave(filename = "groundwarter_singlewell_per_hydrounits_referencecomparison_kartesian_dark_a2.png", width = 59.4, height = 84.1, units = "cm")
...
plot <- data_plot |>
  ...
  theme(
    ...
    plot.subtitle = element_markdown(family = font_subtitle, size = size_subtitle, hjust = 0.5, face = "plain", colour = colour_subtitle_light, lineheight = lineheight_subtitle, margin = margin(0, 0, 80, 0)),
    plot.caption = element_text(family = font_subtitle, colour = colour_subtitle_light, size = 40, hjust = 1, vjust = -20),
    axis.title = element_blank(),
...
plot |>
  ggsave(filename = "groundwarter_singlewell_per_hydrounits_referencecomparison_kartesian_light_a2.png", width = 59.4, height = 84.1, units = "cm")
...
plot_data_legend <- hydrounits |>
  sf::st_join(key_well_meta) |>
  rmapshaper::ms_simplify(keep_shapes = FALSE) |>
...
legend_plot <- plot_data_legend |>
  ...
  ggplot() +
  # geom_sf_pattern(aes(pattern_type = has_well), show.legend = FALSE) +
  geom_sf(aes(fill = has_well), show.legend = FALSE, linewidth = .5, colour = "white") +
  # geom_sf(data = admin_borders_germany, colour = colour_subtitle_light, fill = NA, linewidth = .9) +
  # geom_sf(data = grid_germany, fill = NA) +
  geom_sf(data = key_well_meta, shape = 21, colour = colour_referencecomparison[1], fill = NA, size = 2) +
  # geom_sf_text(aes(label = TR_NR)) +
  ggrepel::geom_text_repel(
    data = plot_data_legend |> drop_na(well_id),
    aes(label = TR_NR, geometry = geometry),
    stat = "sf_coordinates",
    size = 8,
    colour = colour_title_light,
...
legend_plot |>
  ggsave(filename = "legend_minimap_referencecomparison_light_hydrounits.png", scale = 2.5, width = 6, height = 8, units = "cm")
